...
Gröbner bases are in `grobner`: `grobner_basis(&ring, &generators, &order)` runs
...
```rust
use ruffini::grobner::grobner_basis;
...
let basis = grobner_basis(&r, &[x.pow(2) + y.pow(2) - 1, x.clone() - y.clone()], &Lex);
